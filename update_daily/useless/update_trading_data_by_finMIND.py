...
def daily_price_update(filename, today_str, stock_id):
    history_price_data = pd.read_csv(
        os.getcwd()+para.STAlert+para.price_dir+filename, encoding='utf-8-sig', dtype={'stock_id': str})
    # 若重複結束
    ...
    # 撈取今日資料並合併
        ...
        price_data.to_csv(os.getcwd()+para.STAlert+para.price_dir+filename,
...
def daily_chip_update(filename, today_str, stock_id):
    ...
    history_chip_data = pd.read_csv(
        os.getcwd()+para.STAlert+para.chip_dir+filename, encoding='utf-8-sig', dtype={'stock_id': str})
    # 若重複結束
...
def revenue_update(filename, stock_id):
    ...
    if last_date[0:7] == start_date[0:7]:
        ...
        print(os.getcwd()+para.STAlert+para.revenue_dir+filename[0])
        revenue.to_csv(os.getcwd()+para.STAlert+para.revenue_dir+filename,
                       encoding='utf-8-sig', index=None)


if __name__ == '__main__':
    print(os.getcwd()+para.STAlert+'/stock_file_list.csv')
    filename_list = pd.read_csv(os.getcwd()+para.STAlert+'/stock_file_list.csv', encoding='utf-8-sig')
